...
namespace ExtensionMethods.JsonSerializerConverts
{
	...
	/// </summary>
	public class NullableEnumDescriptionJsonConverter : JsonConverter<ValueType?>
	{
		/// <inheritdoc/>
		public override bool CanConvert(Type typeToConvert) => typeToConvert.IsGenericType && typeToConvert.GenericTypeArguments.Length == 1 && typeToConvert.GenericTypeArguments[0].IsEnum;
		/// <inheritdoc/>
		...
		{
			if (value is Enum v)
				writer.WriteStringValue(GetDescription(v));
			else
